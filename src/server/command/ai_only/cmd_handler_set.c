...
int cmd_set_exec(server_t *server, int client_nbr, int offset)
{
	char buff[512];
	...
	if (*inv_resources_ptr == 0)
		return -1;
	*inv_resources_ptr -= 1;
	*cell_resources_ptr += 1;
	snprintf(buff, 512, "pdr %d\n", client_nbr);
	server_graphical_broadcast(server, buff);
	return 0;
}

int cmd_set_callback(server_t *server,
	int client_nbr,
	...
	char **argv)
{
	...
	result = cmd_set_exec(server, client_nbr, cell_helper[i].offset);
	snprintf(buff, 4, "%s\n", result == 0 ? "ok" : "ko");
	write(server->clients[client_nbr].fd, buff, strlen(buff));
	return 0;
}

static int cmd_handler_set_error(server_t *server,
	int client,
	int argc,
	char **argv)
{
...
int cmd_handler_set(server_t *server, int client, int argc, char **argv)
{
	...
	if (cmd_handler_set_error(server, client, argc, argv))
		return 1;
	gettimeofday(&tv, NULL);
	ms = tv.tv_sec * 1000000 + tv.tv_usec;
	trigger_date = ms + (1000000 / server->flag.freq) * 7;
	event_t *evt = new_event(&param, trigger_date, cmd_set_callback);
